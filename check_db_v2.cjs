...
async function check() {
  try {
    ...
    const chartOfAccountsCols = await pool.query("SELECT column_name FROM information_schema.columns WHERE table_name = 'chart_of_accounts'");
    console.log('\nchart_of_accounts columns:');
    chartOfAccountsCols.rows.forEach(row => console.log(' - ' + row.column_name));
