...
async function runE2ETests() {
    console.log('🚀 Starting Comprehensive PCD End-to-End Verification on VPS (Port 5000)...');
    ...
    try {
        ...
        console.log(`✅ Partner ${partnerName} onboarded with ID ${partner.id}`);
        ...
        const product = prodRes.data.data.find(p => p.batches && p.batches.length > 0);
        if (!product) throw new Error('No test products available');

        const orderAmt = 100000;
        ...
        const expectedAmt = orderAmt * 0.96;
        ...
        console.log('\n✨ ALL VPS E2E VERIFICATIONS PASSED SUCCESSFULLY!');
        process.exit(0);
    } catch (error) {
